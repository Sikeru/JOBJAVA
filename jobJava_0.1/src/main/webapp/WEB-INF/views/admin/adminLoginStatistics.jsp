            ...
            <div id="divPaging">
               <ul class="paging">
                  <c:if test="${paging.prev}">
                     <span><a
                        href='<c:url value="loginStatistics.do?page=${paging.startPage-1}"/>'>이전</a></span>
                  </c:if>
                  <c:forEach begin="${paging.startPage}" end="${paging.endPage}"
                     var="num">
                     <span><a
                        href='<c:url value="loginStatistics.do?page=${num}"/>'>${num}</a></span>
                  </c:forEach>
                  <c:if test="${paging.next && paging.endPage>0}">
                     <span><a
                        href='<c:url value="loginStatistics.do?page=${paging.endPage+1}"/>'>다음</a></span>
                  </c:if>
               </ul>
            </div>
         </li>
         ...
         <li id='liSearchOption'>
            ...
            <div class = "search">
               <select name='searchType' id="search">
                  ...
                  <option value='LOGIN' <c:out value="${scri.searchType eq 'LOGIN' ? 'selected' : ''}"/>>로그인시간</option>
                  <option value='LOGOUT' <c:out value="${scri.searchType eq 'LOGOUT' ? 'selected' : ''}"/>>로그아웃시간</option>
               </select> 
   ...
   <script src="http://code.jquery.com/jquery-latest.js"></script>
    ...
    $(function(){
        $('#searchBtn').click(function() {
          self.location = "loginStatistics.do" + '${paging.makeQuery(1)}' + "&searchType=" + $("#search").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
        });
      });   
    </script>
